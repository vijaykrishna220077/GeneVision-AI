...
import { getDb, saveDb, addLog, isSupabaseConfigured, isSupabaseAvailable, supabaseErrorMsg, syncFromSupabase, findUserByEmail, createUser, checkPassword, createSession, getSessionUserId, deleteSession, DEMO_USER_ID } from './server/db';
import { runQuantumSimulation } from './server/quantum';
import { generateChatResponse, generateDiseaseReport, getGeminiClient } from './server/gemini';
import { fetchLiveGeneAnnotations } from './server/geneInfo';
import { DiseaseType, Project, Dataset, PredictionResult, QuantumJob, BiologicalPathway } from './src/types';
...
//
// On Vercel, this file is imported by api/index.ts and run as a serverless
// function. Vercel's own static hosting serves the built frontend (dist/),
// so we must NOT call app.listen() or attach a catch-all static/SPA
// handler in that environment — Vercel sets the VERCEL env var for us.
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`GeneVision AI server booting on port ${PORT}...`);
    addLog('auth', `Server online on port ${PORT}. Environment: ${process.env.NODE_ENV || 'development'}`, DEMO_USER_ID);
  });
}

// Only boot a persistent listener when NOT running as a Vercel serverless
// function (Vercel automatically sets process.env.VERCEL = '1').
